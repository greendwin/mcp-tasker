---
id: s09t07
slug: task-degradation
status: done
---

# Task degradation

Try to convert extended directory-based task to simple file-based.

## Subtasks

- [x] ~~s09t0701: Lets 'reset' command also degrades task if possible~~
- [x] s09t0702: Move should downgrade source story
- [x] s09t0703: Reduce file-based tasks to inline if no subtasks and no description
- [x] s09t0704: Moving all subtasks from parent without description should downgrade it to inline
- [x] s09t0705: Downgrade should not be performed on any other commands except 'move'
- [x] s09t0706: Moving downgrades moved task itself (it can be converted to inline)

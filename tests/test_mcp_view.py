...
def test_list_tasks_returns_taskinfo_instances() -> None:
    create_task("My story")
    result = list_tasks()
    assert isinstance(result[0], TaskInfo)


def test_list_tasks_includes_subtasks() -> None:
    task_id = create_task("My story").task_id
    sub_id = add_subtask(task_id, "First subtask").task_id
    result = list_tasks()
    assert result[0].subtasks is not None
    assert len(result[0].subtasks) == 1
    assert result[0].subtasks[0].id == sub_id
    assert result[0].subtasks[0].title == "First subtask"


def test_list_tasks_subtasks_are_task_previews() -> None:
    task_id = create_task("My story").task_id
    add_subtask(task_id, "First subtask")
    result = list_tasks()
    assert result[0].subtasks is not None
    assert isinstance(result[0].subtasks[0], TaskPreview)

---
id: s26t19
slug: adding-to-todo-must-show
status: pending
---

# Adding to todo must show only updated todo list, not all opened

Note that shown not only todo-list

```
$ t todo s28
Task s28-support-tasks-order added to todo

s12: Ideas
  - s12t12: Shared Tasker (aka 2.0)
  - s12t14: When moving tasks -- update all references ID to moved tasks
  - s12t15: Add 'depends-on' links to tasks
  - s12t18: Support -p suffix, like 'tap' aka parent of 'ta'
  - s12t20: Get rid of sXXtYY ids
  - s12t21: Support merging archived tasks in 'tasker resolve'
  - s12t22: Install shortcuts to .bashrc of .zshrc/.zshuser
  - s12t23: Todo list must be commited to git, don't ignore it
  - s12t24: Order tasks in 'list' by 'depends-on' relations
  - s12t25: Resolve conflicting ids on 'tasker resolve'
  - s12t26: Tasker UI
  - s12t27: Move all local files to .local subdir in .tasker to make it future compatible
s13: Refactoring
  - s13t06: Rework error messages
s19: Usability
  - s19t22: 'tasker add XXX' should open editor with placeholders, same for 'tasker new'
  - s19t32: Interactive mode for 'tasker list'
  - s19t33: Add mpc installation helper 'tasker mcp --install'
  - s19t34: Get rid of 'subtasks' section
s26: [~] Bugs (ta)
  - s26t18: [~] Open tasks list should be grouped
s28: [x] Support tasks order (todo) (q) <<<
```

Note same for `untodo` option: (TBD: what should we show in this case? updated todo list or non-todo tasks?)

```
$ uv run tasker untodo s28
Task s28-support-tasks-order removed from todo

s12: Ideas
  - s12t12: Shared Tasker (aka 2.0)
  - s12t14: When moving tasks -- update all references ID to moved tasks
  - s12t15: Add 'depends-on' links to tasks
  - s12t18: Support -p suffix, like 'tap' aka parent of 'ta'
  - s12t20: Get rid of sXXtYY ids
  - s12t21: Support merging archived tasks in 'tasker resolve'
  - s12t22: Install shortcuts to .bashrc of .zshrc/.zshuser
  - s12t23: Todo list must be commited to git, don't ignore it
  - s12t24: Order tasks in 'list' by 'depends-on' relations
  - s12t25: Resolve conflicting ids on 'tasker resolve'
  - s12t26: Tasker UI
  - s12t27: Move all local files to .local subdir in .tasker to make it future compatible
s13: Refactoring
  - s13t06: Rework error messages
s19: Usability
  - s19t22: 'tasker add XXX' should open editor with placeholders, same for 'tasker new'
  - s19t32: Interactive mode for 'tasker list'
  - s19t33: Add mpc installation helper 'tasker mcp --install'
  - s19t34: Get rid of 'subtasks' section
s26: [~] Bugs
  - s26t18: [~] Open tasks list should be grouped
  - s26t19: Adding to todo must show only updated todo list, not all opened
s28: [x] Support tasks order (q) <<<
```
